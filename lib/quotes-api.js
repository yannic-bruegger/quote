...
export async function getSingleQuote(quoteId, token) {
  const reply = await fetch(`${API_URL}/quotes/${quoteId}?populate=collection`, {
    method: 'GET',
    headers: {
...
export async function updateQuote(token, quote) {
  const reply = await fetch(`${API_URL}/quotes/${quote.quoteId}`, {
    method: 'PUT',
    headers: {
      Authorization: `Bearer ${token}`,
      'Content-Type': 'application/json',
...
export async function deleteQuote(quoteId, token) {
  const reply = await fetch(`${API_URL}/quotes/${quoteId}`, {
    method: 'DELETE',
    headers: {
      Authorization: `Bearer ${token}`,
      'Content-Type': 'application/json',
...
export async function bookmarkCollection(collectionId, token) {
  ...
  const reply = await fetch(`${API_URL}/users/${myUserId}`, {
      method: 'PUT',
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/json',
...
export async function removeBookmarkedCollection(collectionId, token) {
  let collections = (await getMyCollections(token)).follows;
  const collectionIds = collections.map((collection) => collection.id);
